CREATE MATERIALIZED VIEW IF NOT EXISTS stats.v_source_state_local TO stats.source_state_local
  ...
  , SUM(CAST(CASE WHEN event = 'ap.win'                          THEN sign ELSE 0 END AS UInt64)) AS wins
  , SUM(CAST(CASE WHEN event = 'ap.bid'                          THEN sign ELSE 0 END AS UInt64)) AS bids
  , SUM(CAST(CASE WHEN event = 'ap.skip'    AND status IN (0, 1) THEN sign ELSE 0 END AS UInt64)) AS skips
  , SUM(CAST(CASE WHEN event = 'ap.nobid'   AND status IN (0, 1) THEN sign ELSE 0 END AS UInt64)) AS nobids
  , SUM(CAST(CASE WHEN event = 'ap.fail'                         THEN sign ELSE 0 END AS UInt64)) AS errors
  FROM stats.events_local
  WHERE access_point > 0
